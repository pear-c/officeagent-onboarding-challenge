...
LlamaParse는 환경 제약(API Key, 클라우드 전송, docker compose up 한 줄 실행 불가)으로 제외.
대신 **pypdf + 경량 마크다운 변환(정규식 1줄)** 으로 마크다운 인식 청킹을 재활용하여:
- 정답률 유지 (100%)
- 응답 속도 22% 개선
- TS-006(법률 조문 잘림) 해결
- 코드 변경 최소 (+17줄, 외부 의존성 0)

**핵심**: "서버 배포라면 LlamaParse, Docker 한 줄 실행이라면 pypdf + 경량 변환 — 환경 제약에 맞는 최적해를 데이터로 검증했다"
