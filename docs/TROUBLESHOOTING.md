...
**해결 방향**: 05-pdf-enhance에서 번호 헤더(`1. 제목`) 기반 마크다운 변환 → 섹션 단위 청킹 적용 시, 법률 섹션이 통째로 하나의 청크에 포함되어 완화 가능
...
#### Before — pypdf + 재귀 분할 (현재 코드)

| 항목 | 결과 |
|------|------|
| 추출 방식 | pypdf `PdfReader.extract_text()` |
| 청크 수 | <!-- before_chunk_count --> (측정) |
...
| 질문 | 기대 키워드 | pypdf 결과 | 정답 여부 |
|------|-----------|-----------|----------|
| 진단 영역은 몇 개이며 무엇인가? | 4개, 표준/구조/값/관리체계 | <!-- q41_before --> | |
| 진단항목과 진단기준은 각각 몇 개? | 10개, 20개 | <!-- q42_before --> | |
| 법적 근거는? | 공공데이터, 제22조 | <!-- q43_before --> | |
| 시스템 구축 단계는? | 5단계, 계획/발주/설계/개발/완료 | <!-- q44_before --> | |
| 이관 검증 시 컨설팅 요건은? | 조직체계, 절차, 우선순위 | <!-- q45_before --> | |

> 스크린샷: <!-- before_screenshots -->

#### After — LlamaParse + 마크다운 청킹

> 실험 브랜치 `experiment/llamaparse-comparison`에서 진행. 본 코드에 미반영.

| 항목 | 결과 |
|------|------|
| 추출 방식 | LlamaParse API (PDF→Markdown) |
| 청크 수 | <!-- after_chunk_count --> (측정) |
| section 메타데이터 | 헤더 경로 보존 |
| 표 보존 | 마크다운 테이블로 변환 |
| 외부 의존성 | `LLAMA_CLOUD_API_KEY` 필요 |

**Q&A 테스트 (Q41~Q45)**:

| 질문 | 기대 키워드 | LlamaParse 결과 | 정답 여부 |
|------|-----------|----------------|----------|
| 진단 영역은 몇 개이며 무엇인가? | 4개, 표준/구조/값/관리체계 | <!-- q41_after --> | |
| 진단항목과 진단기준은 각각 몇 개? | 10개, 20개 | <!-- q42_after --> | |
| 법적 근거는? | 공공데이터, 제22조 | <!-- q43_after --> | |
| 시스템 구축 단계는? | 5단계, 계획/발주/설계/개발/완료 | <!-- q44_after --> | |
| 이관 검증 시 컨설팅 요건은? | 조직체계, 절차, 우선순위 | <!-- q45_after --> | |

> 스크린샷: <!-- after_screenshots -->

#### 비교 요약

| 메트릭 | pypdf | LlamaParse | 차이 |
|--------|-------|-----------|------|
| 청크 수 | <!-- summary_before_chunks --> | <!-- summary_after_chunks --> | |
| Q&A 정답률 (5문제) | <!-- summary_before_accuracy --> | <!-- summary_after_accuracy --> | |
| section 보존 | X | O | |
| 외부 의존성 | 없음 | API Key 필수 | |
