...
임베딩 모델은 크기보다 정확도를 우선했고, LLM 호출이 느린 부분은 캐시와 스트리밍으로 보완하는 구조입니다. 기술 선택은 직감이 아니라 평가 하네스 측정으로 결정했습니다.
...
RAG 생태계(sentence-transformers, chromadb, pypdf 등)가 Python에 집중되어 있어서 Python을 선택했습니다. FastAPI는 네이티브 async를 지원하므로 LLM 호출(3~15초) 동안 다른 요청을 처리할 수 있고, Pydantic으로 입력 검증과 OpenAPI 문서 생성이 자동으로 됩니다.
...
MTEB 한국어 Retrieval 벤치마크에서 최상위권이라 선택했습니다.
...
bge-m3를 선택한 이유는 한국어 Retrieval 정확도가 높고, 8192 토큰까지 입력을 받아서 긴 청크도 잘리지 않기 때문입니다. e5-base는 512 토큰에서 절단되어 400자 청크도 잘릴 수 있었습니다. 모델 크기가 2.3GB로 무겁지만, 정확도를 우선하는 원칙에 따라 감수했습니다.

### 3.3 벡터 DB — Chroma

이 과제 규모(청크 50~500개)에서는 벡터 DB 간 성능 차이가 사실상 없어서, 셋업이 가장 단순한 Chroma를 선택했습니다.
